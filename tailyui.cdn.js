(() => {
    const components = {
        ...
        '.badge-alt': {
            '@apply text-neutral-50 bg-neutral-800 hover:bg-neutral-700 text-sm w-fit px-3 py-0.5 flex justify-center items-center gap-1 transition-colors rounded-full shadow-sm': {}
        },
        '.badge-ghost': {
    ...
    script.innerHTML = `
        (() => {
            const plugin = tailwind.plugin(function ({addComponents}) {
                addComponents(${JSON.stringify(components)});
            })

            if (!tailwind.config.plugins) tailwind.config.plugins = [];

            tailwind.config.darkMode = 'class';
            tailwind.config.plugins.push(plugin);
        })();
    `;
